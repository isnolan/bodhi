...
describe('chat', () => {
  ...
  // vision: image part, from Google Cloud Storage
  it('vision:image from google cloud storage', async () => {
    const result = await api.sendMessage({
      model: 'gemini-pro-vision',
      messages: [
        {
  ...
  it('function call', async () => {
    const result = await api.sendMessage({
      model: 'gemini-pro',
      messages: [{ role: 'user', parts: { text: 'Which theaters in Mountain View show Barbie movie?' } }],
      tools: [
        {
