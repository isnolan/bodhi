...
var OpenAICompletionsAPI = class extends ChatBaseAPI {
  ...
  async sendMessage(opts) {
    ...
    return new Promise(async (resolove, reject) => {
      ...
      if (!res.ok) {
        ...
        throw new chat.ChatError(((_a = reason.error) == null ? void 0 : _a.message) || 'request error', res.status);
      }
      let response;
      const body = res.body;
        ...
        if (event.type === 'event') {
          if (event.data === '[DONE]') {
            ...
            try {
              ...
              onProgress == null ? void 0 : onProgress(response);
            } catch (e) {}
          }
        }
      });
  ...
  convertParams(opts) {
    return {
      ...
      //   },
      // ],
      temperature: (opts == null ? void 0 : opts.temperature) || 0.9,
      top_p: (opts == null ? void 0 : opts.top_p) || 1,
      n: opts.n || 1,
      max_tokens: (opts == null ? void 0 : opts.max_tokens) || null,
      stop: (opts == null ? void 0 : opts.stop_sequences) || null,
      stream: true,
    };
  }
};
    ...
    return new Promise(async (resolove, reject) => {
      ...
      const parser = createParser2((event) => {
        if (event.type === 'event') {
          ...
          onProgress == null ? void 0 : onProgress(choices);
...
var GoogleVertexAPI = class extends GoogleGeminiAPI {
  ...
  async sendMessage(opts) {
    ...
    return new Promise(async (resolove, reject) => {
      ...
      body.on('end', () => {
        const choices = this.combineChoices(choicesList);
        resolove({ id: uuidv42(), model: opts.model, choices, useage });
      });
    });
  }
};
...
var AnthropicClaudeAPI = class extends ChatBaseAPI {
  ...
  async sendMessage(opts) {
    ...
    return new Promise(async (resolove, reject) => {
      ...
      if (!res.ok) {
        const reason = await res.json();
        throw new chat.ChatError(((_a = reason[0].error) == null ? void 0 : _a.message) || 'request error', res.status);
      }
      let response;
      const body = res.body;
      body.on('error', (err) => reject(new chat.ChatError(err.message, 500)));
...
var dist_es_exports = {};
__reExport(dist_es_exports, signature_v4_star);
import * as signature_v4_star from '@smithy/signature-v4';
...
async function requestWithAuth(options, request) {
  ...
  const signed = await new dist_es_exports.SignatureV4({ region, service, sha256: Sha256, credentials }).sign(
...
var AnthropicBedrockAPI = class extends ChatBaseAPI {
  ...
  async sendMessage(opts) {
    ...
    return new Promise(async (resolove, reject) => {
      ...
      if (!res.ok) {
        const reason = await res.json();
        throw new chat.ChatError(reason.message || 'request error', res.status);
      }
      let response;
      const body = res.body;
      body.on('error', (err) => reject(new chat.ChatError(err.message, 500)));
...
var AliyunQwenAPI = class extends ChatBaseAPI {
  ...
  async sendMessage(opts) {
    ...
    return new Promise(async (resolove, reject) => {
      ...
      body.on('end', () => {
        const choices = this.combineChoices(choicesList);
        resolove({ id: uuidv43(), model: opts.model, choices, useage });
      });
    });
  }
  /**
  ...
  async sendMessage(opts) {
    ...
    return new Promise(async (resolove, reject) => {
      ...
      for await (let message of response) {
        ...
        onProgress == null ? void 0 : onProgress(choices2);
