...
import fetchSSE11 from 'node-fetch';
import { v4 as uuidv48 } from 'uuid';
import { HttpsProxyAgent as HttpsProxyAgent12 } from 'https-proxy-agent';
import { createParser as createParser10 } from 'eventsource-parser';
var GroqCompletionsAPI = class extends ChatBaseAPI {
  ...
  async sendMessage(opts) {
    ...
    return new Promise(async (resolove, reject) => {
      const url = `${this.baseURL}/chat/completions`;
      const params = await this.convertParams(options);
      const res = await fetchSSE11(url, {
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${this.apiKey}` },
        body: JSON.stringify(params),
        agent: this.agent ? new HttpsProxyAgent12(this.agent) : void 0,
        method: 'POST',
      });
      if (!res.ok) {
        const reason = await res.json();
      ...
      if (params.stream === false) {
        ...
        const choices = this.convertChoices(result.choices);
        const usage = result?.usage;
        resolove({ id: uuidv48(), model: opts.model, choices, usage });
      } else {
        const body = res.body;
        body.on('error', (err) => reject(new chat.ChatError(err.message, 500)));
        let result;
        const choicesList = [];
        const parser = createParser10((event) => {
          if (event.type === 'event') {
            if (event.data !== '[DONE]') {
              try {
                result = JSON.parse(event.data);
